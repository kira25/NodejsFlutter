...
class _LoginPageState extends State<LoginPage> {
  ...
  signUp(String username, email, pass) async {
    ...
    var response =
        await http.post("http://192.168.1.9:3000/signup", body: data);
    if (response.statusCode == 200) {
      jsonResponse = json.decode(response.body);
      if (jsonResponse != null) {
        setState(() {
            ...
            onPressed: () {
              if (emailController.text == "" ||
                ...
                if (_isregistered == false) {
                  ...
                  return signIn(emailController.text, passwordController.text);
                } else if (_isregistered == true || usernameController.text !="") {
                  setState(() {
                    _isloading = true;
                  });
                  return signUp(usernameController.text, emailController.text,
                      passwordController);
                }
              }
              return null;
